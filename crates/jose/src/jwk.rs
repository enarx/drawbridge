...
pub struct JwkSet<P = Parameters> {
    pub keys: Vec<Jwk<P>>,
}
...
pub enum Key {
    #[serde(rename = "EC")]
    EllipticCurve {
        crv: EllipticCurveType,

        #[serde(skip_serializing_if = "Option::is_none", default)]
        d: Option<Bytes<Zeroizing<Vec<u8>>>>,
    ...
    Rsa {
        ...
        prv: Option<RsaPrivate>,
    },

    #[serde(rename = "oct")]
    Octets { k: Bytes<Zeroizing<Vec<u8>>> },

    #[serde(rename = "OKP")]
    OctetKeyPair {
        crv: OctetKeyPairType,
...
pub struct Parameters {
    ...
    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub key_ops: Option<BTreeSet<Operations>>,

    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub x5u: Option<Url>,

    #[serde(skip_serializing_if = "Option::is_none", default)]
    pub x5c: Option<Vec<Bytes<Vec<u8>, Standard>>>, // base64, not base64url
...
pub struct RsaPrivate {
    pub d: Bytes<Zeroizing<Vec<u8>>>,
    pub p: Bytes<Zeroizing<Vec<u8>>>,
    pub q: Bytes<Zeroizing<Vec<u8>>>,
    pub dp: Bytes<Zeroizing<Vec<u8>>>,
    pub dq: Bytes<Zeroizing<Vec<u8>>>,
    pub qi: Bytes<Zeroizing<Vec<u8>>>,
...
pub struct RsaOtherPrimes {
    pub r: Bytes<Zeroizing<Vec<u8>>>,
    pub d: Bytes<Zeroizing<Vec<u8>>>,
    pub t: Bytes<Zeroizing<Vec<u8>>>,
}
...
mod rfc7517 {
    ...
    fn a1() {
        ...
        let jwk = JwkSet {
            keys: vec![
                Jwk {
                    key: Key::EllipticCurve {
                        crv: EllipticCurveType::P256,
                        d: None,
                        x: Bytes::from(vec![
        ...
        let jwk = JwkSet {
            keys: vec![
                Jwk {
                    key: Key::EllipticCurve {
                        crv: EllipticCurveType::P256,
                        d: Some(Bytes::from(Zeroizing::from(vec![
                ...
                Jwk {
                    key: Key::Rsa {
                        ...
                        prv: Some(RsaPrivate {
                            d: Bytes::from(Zeroizing::from(vec![
                            ...
                            p: Bytes::from(Zeroizing::from(vec![
                                243, 120, 190, 236, 139, 204, 25, 122, 12, 92, 43, 36, 191, 189,
                                211, 42, 191, 58, 223, 177, 98, 59, 182, 118, 239, 59, 252, 162,
                                62, 169, 109, 101, 16, 200, 179, 208, 5, 12, 109, 61, 89, 240, 15,
                                109, 17, 251, 173, 30, 76, 57, 131, 218, 232, 231, 50, 222, 79,
                                162, 163, 43, 155, 196, 95, 152, 216, 85, 88, 59, 99, 140, 201,
                                130, 50, 51, 169, 73, 120, 156, 20, 120, 251, 92, 235, 149, 33,
                                132, 50, 169, 85, 165, 88, 72, 122, 116, 221, 250, 25, 86, 88, 147,
                                221, 205, 240, 23, 61, 189, 142, 53, 199, 47, 1, 245, 28, 243, 56,
                                101, 80, 205, 123, 205, 18, 249, 251, 59, 73, 213, 109, 251,
                            ])),
                            q: Bytes::from(Zeroizing::from(vec![
                                221, 215, 206, 71, 215, 46, 98, 175, 180, 75, 233, 164, 20, 188,
                                224, 34, 216, 12, 17, 241, 115, 7, 106, 183, 133, 103, 161, 50,
                                225, 180, 160, 43, 170, 157, 189, 239, 161, 178, 242, 186, 106,
                                163, 85, 148, 14, 213, 210, 43, 119, 8, 19, 156, 39, 105, 99, 48,
                                92, 57, 245, 185, 175, 126, 244, 0, 85, 227, 137, 103, 237, 252,
                                209, 132, 138, 139, 232, 158, 44, 225, 42, 154, 61, 85, 84, 187,
                                241, 60, 197, 131, 25, 8, 118, 183, 156, 69, 236, 236, 103, 237,
                                100, 97, 223, 236, 214, 160, 219, 198, 217, 3, 18, 7, 192, 33, 48,
                                6, 244, 181, 39, 0, 59, 167, 226, 242, 28, 111, 172, 158, 151, 25,
                            ])),
                            dp: Bytes::from(Zeroizing::from(vec![
                                27, 139, 15, 94, 71, 58, 97, 175, 114, 242, 130, 86, 247, 242, 11,
                                143, 140, 110, 166, 155, 180, 151, 56, 191, 31, 181, 83, 145, 47,
                                49, 143, 148, 157, 95, 119, 40, 19, 74, 34, 153, 140, 49, 34, 45,
                                158, 153, 48, 46, 123, 69, 14, 107, 151, 105, 128, 81, 178, 4, 158,
                                28, 242, 212, 54, 84, 94, 52, 217, 116, 110, 128, 160, 211, 63,
                                198, 164, 98, 17, 104, 230, 208, 0, 239, 180, 30, 252, 217, 173,
                                185, 134, 92, 220, 45, 230, 220, 141, 184, 27, 97, 175, 71, 155,
                                18, 15, 21, 50, 0, 221, 179, 171, 194, 223, 159, 209, 20, 154, 206,
                                171, 99, 115, 155, 241, 135, 162, 42, 68, 226, 6, 61,
                            ])),
                            dq: Bytes::from(Zeroizing::from(vec![
                                179, 217, 64, 31, 215, 224, 128, 27, 40, 21, 31, 14, 105, 205, 145,
                                252, 77, 160, 195, 111, 54, 173, 61, 164, 24, 224, 33, 188, 137,
                                101, 17, 49, 53, 121, 250, 192, 234, 27, 148, 82, 243, 31, 5, 195,
                                41, 159, 201, 106, 121, 110, 175, 207, 57, 216, 99, 148, 146, 64,
                                94, 233, 49, 208, 191, 106, 2, 55, 156, 111, 8, 110, 157, 65, 81,
                                189, 9, 82, 42, 218, 68, 218, 148, 124, 184, 92, 65, 191, 221, 244,
                                97, 120, 14, 30, 222, 239, 133, 155, 70, 202, 27, 70, 137, 238,
                                141, 54, 13, 215, 16, 154, 63, 164, 206, 235, 88, 239, 90, 181,
                                254, 47, 95, 45, 197, 124, 56, 247, 132, 63, 114, 9,
                            ])),
                            qi: Bytes::from(Zeroizing::from(vec![
                                27, 35, 63, 167, 162, 107, 95, 36, 162, 207, 91, 104, 22, 2, 155,
                                89, 95, 137, 116, 141, 227, 67, 140, 169, 187, 218, 219, 49, 108,
                                119, 173, 2, 65, 126, 107, 116, 22, 134, 51, 129, 66, 25, 17, 81,
                                68, 112, 234, 176, 122, 100, 77, 243, 92, 232, 12, 6, 154, 248, 25,
                                52, 41, 99, 70, 14, 50, 71, 100, 55, 67, 152, 88, 86, 220, 3, 123,
                                148, 143, 169, 187, 25, 63, 152, 118, 70, 39, 93, 107, 199, 36,
                                124, 59, 158, 87, 45, 39, 183, 72, 249, 145, 124, 172, 25, 35, 172,
                                148, 219, 134, 113, 189, 2, 133, 96, 139, 93, 149, 213, 10, 27, 51,
                                186, 33, 174, 179, 76, 168, 64, 85, 21,
                            ])),
                            oth: vec![],
                        }),
                    },
...
mod rfc8037 {
    ...
    fn a1() {
        ...
        let jwk = Jwk {
            key: Key::OctetKeyPair {
                crv: OctetKeyPairType::Ed25519,
                d: Some(Bytes::from(Zeroizing::from(vec![
